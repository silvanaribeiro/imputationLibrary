...
def inputData(x, y, x_new, derivate_order=0):
    tck = interpolate.splrep(x, y, s=derivate_order)
...
def inputTrainingData(df, derivate_order=0):
    ...
    for col, data in training_df.iteritems():
        ...
        if(len(x_new)>0):
            y_new = inputData(complete_df.index.values.tolist(), complete_df.iloc[:].values.tolist(), x_new.tolist(), derivate_order)
